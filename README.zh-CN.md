...
本 fork 的 `production` 分支新增[订阅分发](docs/features/distribution.zh-CN.md)：固定用户链接、试用与卡密续期、可调为 1／2 个城市的地区限制及公开自助页。可粘贴客户完整订阅链接或完整卡密精准定位记录。订阅列表支持逐条一键复制（自动识别或指定原生格式）、单条及批量删除，删除保留兑换记录和卡密绑定；Loon 自动识别使用原生节点列表，无需 Sub-Store。拒绝提示分条展示，到期提示支持自定义多条节点文案和顺序。卡密支持编辑、启用／停用、批量删除和确认补发免费卡，保留已发放权益与兑换记录。可选的 ip2region、ipdata 城市兜底补足 GeoLite2 覆盖，不放宽地区限制。管理后台位于 `/admin`，`/` 为公开首页；`main` 用于跟踪上游。
