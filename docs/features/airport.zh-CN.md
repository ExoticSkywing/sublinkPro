...
每次订阅拉取任务完成后，任务进度面板和「任务中心」历史记录会提供「查看过滤明细」操作。报告包含：

- 上游订阅返回的原始节点总数。
- 被全局规则过滤的数量、被当前机场规则过滤的数量、过滤总数和保留数量。
- 每个被过滤节点的原始名称、协议、过滤阶段和原因，并支持按阶段和关键词筛选。

报告按以下顺序分阶段统计：全局过滤 → 机场过滤。全局规则已经过滤掉的节点不会再次计入机场规则。报告中的「保留」是两个过滤阶段之后的数量，不包含后续去重删除的节点；明细中的节点名称是重命名前的上游原始名称。

报告会长期保存在机场记录中，机场管理页面会直接展示最近一次成功拉取的摘要，并可从机场卡片、表格或移动端卡片打开过滤明细；同时也会保存到本次订阅更新任务结果中，方便查看历史拉取。下一次拉取成功后会覆盖机场记录中的最近一次报告；拉取失败时保留上一次成功报告。清理任务历史只会删除任务历史中的报告，不会影响机场页面上的最近一次报告；删除机场时该报告也会与机场记录一并删除。
