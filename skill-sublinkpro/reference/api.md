...
## Subscription distribution (production fork)

Independent from legacy shares: stable recipient links use `/d/<64-hex-token>`.
Management base: `/api/v1/distribution`, existing administrator JWT or `X-API-Key` authentication. API keys retain existing administrator-level privileges; do not expose them in public pages or client applications.

Success envelope: `{"code":200,"data":...}`. Lists: `data.items` and `data.total`, query `page` (1-based), `size` (default 20, max 100). Credential/card search accepts `keyword` (full link/code or name/batch). Credentials, cards and requests accept `status`. Visits, requests and redemptions accept `credential_id`.

For recipient-link lookup use authenticated **POST `/credentials/search`** with JSON `{keyword, page?, size?, status?}`; do not put secret links in GET query strings or request-body logs. The trimmed keyword is limited to 2048 bytes without silent truncation. Full HTTP(S) `/d/<token>` links (including query/fragment) and complete 64-hex tokens match the indexed token hash exactly; name/batch text search remains available. Status filters and pagination are identical to GET; deleted links require `status=revoked` and omit the token. Lookup never activates or changes rights/cities. Rotated-out tokens are not retained and cannot locate the original record. URLs are parsed locally, never fetched; invalid/unmatched links produce an empty page.

| Method / suffix | Input / result |
| --- | --- |
| GET `/settings` | Full business settings |
| PUT `/settings` | Full settings object (read, edit, write) |
| GET `/credentials` | Paginated credentials including authorized export tokens and allowed regions; default excludes `revoked`; `status=enabled|disabled|revoked` filters explicitly; revoked rows omit tokens |
| POST `/credentials/search` | Read-only credential search using JSON `{keyword, page?, size?, status?}`; full link/token exact match or name/batch search; returns the same page shape as GET |
| POST `/credentials` | `{name, batch?, count, subscription_id}`; returns array with `id`, `token`, `status`, `trial_days`, etc. |
| GET `/credentials/:id` | Credential and regions; revoked/deleted records remain available for read-only audit, without token |
| PATCH `/credentials/:id` | Optional `{name, subscription_id, status, rotate, permanent, expires_at}`; returns updated credential, token and regions; enabled/disabled/revoked; rotation invalidates old token; rights changes require prior activation |
| DELETE `/credentials/:id` | Permanently invalidate a link using the existing `revoked` state; preserve redemption history and paid-code ownership; safe to repeat |
| POST `/credentials/batch-delete` | `{ids: [1,2]}`; 1–100 unique positive IDs; atomic deletion, any missing ID rejects the batch |
| GET `/cards` | Paginated cards including export codes |
| POST `/cards/search` | Read-only card search using JSON `{keyword, page?, size?, status?}`; exact full-code or name/batch search; same page shape as GET |
| GET `/cards/:id` | Authenticated card details, including `deleted`; deleted cards are read-only audit records with `code` omitted |
| POST `/cards` | `{name, batch?, count, kind, ends_at?}`; kind quarter/year/permanent; returns card array with `code` |
| PATCH `/cards/:id` | Partial `{name?, kind?, ends_at?, enabled?}`; at least one known field; `ends_at: null` clears a paid deadline, omission preserves it |
| DELETE `/cards/:id` | Soft-delete/invalidate a code, preserving granted benefits and redemption history; repeating deletion is safe |
| POST `/cards/batch-delete` | `{ids: [1,2]}`; 1–100 unique positive IDs, atomic; missing ID rejects the batch |
| POST `/cards/free-cycle` | No body required; atomically obtains/creates the current shared free code |
| POST `/cards/free-cycle/reissue` | Explicit replacement of a deleted current-cycle code; no body; repeats return existing live/disabled record, never re-enable it |
| POST `/status` | `{link}`; sanitized recipient status, regions, `region_limit`, recent denied candidates and request history |
| POST `/redeem` | `{link, code}`; returns original/new redemption record with `id`, `credential_id`, `card_id`, `kind`, `before`, `after`, `permanent`, `source`, `created_at`, `already_redeemed` |
| GET `/redemptions` | Paginated redemption history, adding `credential_name`, `credential_deleted`, `card_name`, `card_deleted`; no link tokens or codes |
| GET `/visits` | Paginated recent visits including IP, UA, city and decision |
| GET `/region-requests` | Paginated requests, with reason/contact/reply |
| POST `/region-requests` | `{link, visit_id, reason, contact?}`; visit must be this credential's CN city denial within 7 days |
| POST `/region-requests/:id/review` | `{approve, replace_region_id?, reply?}`; replacement required at the configured city limit; rejection requires reply |

Settings fields: `trial_days` (15), `free_days` (7), `cycle_days` (7), `cycle_anchor` (RFC3339 UTC, initial UTC day), `domain` (hostname only), `portal_url` (absolute HTTP(S), no query/fragment), `allowed_ua` (Go regexp), `fallback_subscription_id` (0 = notices only), `expired_message`. Days accept 1–365. Read defaults via GET; modifying cycle settings changes code boundaries. An empty domain uses the browser origin for exported links; managed-update links prefer configured domain, otherwise request host with HTTPS.

`region_limit` accepts 1 or 2 (default: 2). Use 1 for temporary city-limit testing; restore 2 afterwards. Omission/null/zero in older settings requests preserves the current value. Other values are invalid. Changes apply to both preflight and final delivery checks and region approvals. Lowering below an undeleted subscription's registered-city count rejects the entire write with `region_limit_conflict`, without deleting registrations. Startup migration sets legacy instances to 2 and preserves saved values on subsequent runs. This does not implement a physical-device limit.

For code lookup use authenticated POST `/cards/search`, keeping secrets out of query URLs and request-body logs. Trimmed keywords are limited to 2048 bytes; full codes match the indexed digest using redemption normalization: new 16-character codes accept either case, optional hyphens and ASCII whitespace; legacy 64-hex codes accept either case and surrounding whitespace. Exact code lookup never mixes fuzzy name matches or scans all ciphertexts. Partial codes do not search code contents. Non-code keywords retain name/batch text search. `status=enabled|disabled` and pagination remain effective, deleted cards are excluded, and searches never redeem or change cards/rights.

Credential deletion uses `revoked`, not a new database state. Legacy PATCH revocation remains supported and has the same deletion semantics; its response omits the token. Deleted rows cannot be restored, edited, rotated, renewed or redelivered by an original-batch retry (`conflict`). Audit references, activation, cities, expiry and paid-card ownership remain intact; historical paid cards never become available to another recipient. Normal `disabled` links can still be re-enabled. All deletion endpoints require administrator JWT/API-key authentication. Client visit results distinguish `inactive` (disabled) from `revoked` (permanently deleted). Built-in denial reasons, recovery actions and portal URLs render as separate unusable notice nodes; disabled/deleted responses never inherit expiry fallback resources.

The default `allowed_ua` includes the explicit `ClashMetaForAndroid` client identifier with the existing name-boundary check. Such requests select Clash YAML without a `client` query override. Startup upgrades only the exact previous default regexp; custom allowlists remain authoritative and are not replaced. This compatibility entry does not bypass IP/city/expiry/status checks. A rejected UA still returns HTTP 403 with `Unsupported subscription client` and records `ua_denied` for a valid link.

`expired_messages` is an optional ordered array of 1–10 nonempty single-line strings (maximum 200 Unicode characters each after trimming; control characters are rejected). Example: `"expired_messages": ["Subscription expired", "Redeem a code to renew", "{portal}"]`. Each item produces one unusable notice node; `{portal}` expands to the complete public portal URL. No URL is implicitly appended to configured items. Duplicate expanded names get numeric suffixes. The array takes precedence over legacy `expired_message`; GET returns `null` for legacy settings, which render the legacy message and portal as two separate nodes. Omitting the array or sending `null` preserves any saved list; an empty array is invalid. New callers supplying the array may omit `expired_message`; old callers must still supply that field (1–200 bytes). Saving notices changes only expired responses, not other denials or the selected fallback policy. Clients see changes on their next refresh.

Credential `subscription_id` updates require an existing, non-deleted resource subscription with a positive ID. Omission leaves the resource unchanged. Switching is allowed before/after activation but not after revocation; it preserves the token, activation, rights, expiry and cities unless those other fields are explicitly patched. The next client refresh uses the replacement resources. A resource switch before final delivery authorization discards the already-rendered old response with HTTP 503 and does not activate or register a city. Successful and notice responses use the credential name as their filename and `profile-title` (`base64:` UTF-8); clients may retain their previously imported local name.

All timestamps accept RFC3339; optional deadlines may be null. Batch count is 1–100. For counts above one, credential/card names append a three-digit suffix (`name-001`, `name-002`, …); single-item names stay unchanged. Existing names are not migrated. Repeating an explicit batch returns original items; conflicting count/resource/kind returns `conflict`. Keep each batch ID unique to one delivery operation. Paid cards bind to one credential forever. Duplicate redemption returns the original record, without extension. Free codes are reusable by different credentials, once per credential per UTC cycle, without stacking or shortening active trial/paid rights. Activate before redeeming. Current free code generation is on demand, not a background schedule; call the endpoint from automation each cycle. A disabled current code remains disabled.

Both redemption APIs retain HTTP 200 for safe idempotent retries. `already_redeemed: false` means this request granted benefits; `true` means it returned the original redemption with no additional renewal. This response-only flag is not stored. Display original `created_at` for duplicate feedback; do not treat historical `after` as the current subscription expiry, since administrators or subsequent redemptions may have changed it. Query `/status` for current state. History names are current names (not snapshots); deleted-card names remain available, and missing related rows produce empty names for an ID-label fallback. Neither history nor deleted-card details exposes secret/code/token fields. Live card details keep the same authorized export-code access as the card list.

Newly issued card `code` values use 16 random letters/digits (80 bits) grouped `XXXX-XXXX-XXXX-XXXX`. Both redemption APIs accept either case, optional hyphens and ASCII whitespace. Existing 64-character codes, batch retries and already-issued current free codes are preserved; clients must support both formats. Credential tokens remain 64 hex characters. Card secrets remain restricted to authenticated management responses.

Cards accept `status=enabled|disabled` filtering and exclude deleted records. Card PATCH preserves the code and validates a nonblank name (up to 100 UTF-8 bytes); a changed paid deadline must be in the future. Only unused paid cards can change between quarter/year/permanent. Redeemed cards retain type/owner; free cards retain type/cycle/deadline (`card_locked` otherwise). Name edits remain available. These operations never rewrite granted benefits or redemption history. Deleted codes are unavailable even to previous redeemers, cannot be enabled, and make original-batch issuance retries return `conflict`. Soft-deleted records retain their encrypted code and audit metadata in the database, but are excluded from management lists. A deleted current free code makes ordinary retrieval return `card_deleted`; explicit reissue creates a different code in the same cycle. Original redemption records enforce once-per-link-per-cycle across all replacements. Next-cycle retrieval works normally. All new management endpoints require the same admin JWT/API-key authentication as existing card endpoints.

Public base: `/api/public/distribution` (no administrator authentication): GET `/settings` exposes only `portal_url`, `trial_days`, `free_days`, `region_limit`; POST `/status`, `/redeem`, `/region-requests` use the same payloads above. The link acts as the bearer credential; callers may supply the full HTTP(S) `/d/` URL or the exact token. URLs are parsed, never fetched. Public status omits raw IP/UA. Keep tokens/codes in POST bodies, never put them in public query strings or logs.

Successful distribution `/d/` responses follow the current resource subscription's `RefreshUsageOnRequest` switch. False: no usage refresh and no `subscription-userinfo`, including cached or zero values. True: reuse the existing airport usage refresh/aggregation and return usage; the `expire` field is replaced with the recipient's expiry (omitted for permanent rights), not the airport's. Traffic remains shared resource-pool data, not an individual quota. Browser/HEAD probes and denial/expiry notices (including fallback nodes) do not refresh or expose usage. Recipient `/status` still exposes expiry independently of this switch. Existing admin subscription edits control the flag; no new distribution setting or API field is added. Legacy `/c/` usage behavior is unaffected.

Consumer: GET/HEAD `/d/:token` with a recognized client UA and optional `client` format. Omit `client` for automatic detection; the management copy menu also offers native `clash`, `mihomo`, `surge` and `v2ray`. All formats retain the same token and policy checks. Browser UA redirects to portal even with a format override. HEAD never activates. Valid CN-city GET plus successful nonempty rendering activates the trial and registers cities up to `region_limit` (default 2). Cities beyond the limit receive only unusable notices; recipients request city replacement through the portal. GeoIP/UA is a risk heuristic, not identity proof or device identification. Expired links return configured fallback nodes or notices; disabled/foreign/unlocated/over-limit-city requests never receive fallback resources. Loon and Shadowrocket auto-detection uses the native Base64 URI list (`v2ray`), without Sub-Store. Explicit `client=loon` and other expanded formats still require configured Sub-Store. Update interval is 24 hours; recipients must configure their client accordingly.

Business errors use HTTP 400 and `i18nKey: distribution.errors.<code>`: `invalid_input`, `not_found`, `inactive`, `not_activated`, `card_unavailable`, `card_used`, `card_locked`, `card_deleted`, `already_covered`, `conflict`, `too_frequent`, `region_limit_conflict`. Unexpected errors return 500. Consumer generation failures return 503; unsupported UA returns 403. Public per-instance IP limits return 429 plus `Retry-After: 60`: portal 30/min, pull 120/min. Region applications additionally allow one pending request and three new requests/24h per link. Management authentication uses existing 401/403 behavior.

City resolution uses a bounded 24-hour IP cache → GeoLite2 → optional IPv4/IPv6 ip2region → optional ipdata. Provider aliases map to existing GeoNames city keys; unknown/ambiguous mappings and conflicting known locations fail closed. Foreign results are terminal. Invalid links, HEAD, unsupported UAs and disabled links skip external lookups. Missing cities, timeouts and quota failures do not activate or reserve cities. Fallback providers do not weaken the configured city limit; ipdata secrets are deployment-only, never exposed through these APIs. See [configuration](../../docs/configuration.md#distribution-city-fallback-production-fork).

Example renewal (replace the illustrative link/code):

```bash
curl --request POST 'https://sub.example.com/api/public/distribution/redeem' \
  --header 'Content-Type: application/json' \
  --data '{"link":"https://sub.example.com/d/REPLACE_WITH_TOKEN","code":"REPLACE_WITH_CARD"}'
```

Automation: issue credentials via authenticated POST `/credentials`, keep the returned ID/link in the delivery system, obtain cycle codes through authenticated POST `/cards/free-cycle`, and redeem via `/redeem`. Database and encryption-key backup requirements are in `docs/features/distribution.md` (Chinese: `.zh-CN.md`). Existing legacy database import refuses sources/targets containing distribution records before clearing any data; use full-database restore.
